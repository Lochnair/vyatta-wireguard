...
define gen_deb
	...
	sed -i "s@KERNEL_VER@$$(find $(1)/lib/modules/ -maxdepth 1 -mindepth 1 -type d -printf "%f\n")@g" package/scratch/debian/preinst
        ...
        $(AR) -rcs package/$(shell sed -n 's/Version: \(.*\)/wireguard-v2.0-$(1)-\1.deb/p' debian/control) package/scratch/debian-binary package/scratch/control.tar.gz package/scratch/data.tar.gz
